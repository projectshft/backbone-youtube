var VideosCollection = Backbone.Collection.extend({
  ...
  addVideo: function (title, description, id) {
    this.add({
      title: title,
  ...
  getResults: function (query) {
    var search_params = {
      'key': 'AIzaSyBW1oAZn9yrQDhfoZPkwnHvVNDUsJRe7Ho',
      'q': query,
      'type': 'video',
      'limit': 5
    };
...
// Response is parsed after every fetch event.  
    ...
    return response.items.map(function (v) {
      if (v.id.kind == 'youtube#video') {
        var newObj = {
          ...
          thumbnail_link: v.snippet.thumbnails.default.url
        }

        return Object.assign({}, v, newObj);
